...
## 5. 右ペイン仕様（Findings / Audience）

### タブ
- Findings（デフォルト）
- Audience（内部ID: `persona`）
- 選択状態：下線（青）＋文字色変化
...
- `AppShell`
  ...
  - `MainSplitPane`
    - `EditorPane`
      ...
      - `MarkdownPreview`（Previewモード時）
    - `ResultsPane`
      - `Tabs`（Findings / Audience）
      - `FindingsView`
        - `VerdictBanner`
        - `SeverityFilterChips`
        - `FindingCardList`
          - `FindingCard`
      - `PersonaView`（Audienceタブの内容）
  - `ExportModal`（Export成功時に表示）
  - `Toast`（通知表示）
...
#### 用途
- クリップボードコピー成功時などのフィードバック表示

#### 表示仕様
- 画面下部中央に固定表示（`fixed bottom-4`）
- 背景: 半透明黒（`bg-gray-800/90`）
- テキスト: 白
- 角丸: `rounded-lg`
- フェードインアニメーション付き
...
#### 適用箇所
- **SettingsBar**: 折りたたみ時のサマリ表示（例: `公開 / テクニカル / 技術者`）
- **PersonaView**: Audience表示（例: `Audience: 技術者`）
...
#### タブ名
- 右パネルのタブ名は **Findings** / **Audience** とする
- ※ 内部ID (`persona`) やAPI パラメータは変更しない

#### メッセージ（Audienceタブ内）
- ローディング中: `Running audience review...`
- 待機中: `Audience review will start automatically.`
- 結果なし: `No audience review results available.`
