...
技術ブログ公開前チェックツール（MVP）の仕様と動作方法について記述します。
Google Gemini 2.5 Flash を利用して、Markdown コンテンツのセキュリティ、プライバシー、法的リスクを分析します。

## 概要
ユーザーが執筆した Markdown 記事に対して、以下の観点でリスク評価を行います。
- **Security**: 認証情報や内部情報の漏洩リスク
- **Privacy**: 個人情報の不適切な記載
- **Legal/Compliance**: 法的・コンプライアンス上の懸念
- **Tone & Audience**: 記事のトーンや想定読者に適しているか
...
```
.
├── blog-risk-checker/    # Frontend (React) アプリケーション
│   ├── src/
│   │   ├── api/          # バックエンドとの通信 (client.ts)
│   │   ├── store/        # Zustand ストア (appStore.ts)
│   │   ├── components/   # UIコンポーネント
│   │   └── types/        # 型定義
│   └── package.json
├── main.py               # Backend (FastAPI) アプリケーション
├── CLAUDE.md             # AIコーディングアシスタント用ガイド（英語）
└── README.md             # 本ファイル
```
